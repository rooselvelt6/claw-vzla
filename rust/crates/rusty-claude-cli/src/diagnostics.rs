...
use runtime::{
    load_oauth_credentials, resolve_sandbox_status, ConfigLoader, ConfigError, ProjectContext,
    RuntimeConfig, SandboxStatus,
};
use serde_json::{json, Map, Value};

use super::{
    parse_git_status_metadata, parse_git_workspace_summary, CliOutputFormat, StatusContext,
    DEFAULT_DATE, OFFICIAL_REPO_SLUG, OFFICIAL_REPO_URL, DEPRECATED_INSTALL_COMMAND, VERSION,
    BUILD_TARGET, GIT_SHA,
};
...
pub(super) fn render_doctor_report() -> Result<DoctorReport, Box<dyn std::error::Error>> {
...
pub(super) fn run_doctor(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
...
fn check_config_health(
    ...
    config: Result<&RuntimeConfig, &ConfigError>,
) -> DiagnosticCheck {
...
fn check_sandbox_health(status: &SandboxStatus) -> DiagnosticCheck {
...
fn check_system_health(cwd: &Path, config: Option<&RuntimeConfig>) -> DiagnosticCheck {
